...
直接用 ChatGPT「一步翻译」，产出的是**字面忠实但读起来像外语**的文本。这个 Skill 把它拆成一条有明确交付物的流水线，并且补上了通用翻译提示词普遍缺失的关键一环 —— **本土化编辑**。

| 段 | 角色 | 关注点 |
| --- | --- | --- |
| 1 | 翻译专家 | 忠实 + 自然，出初稿 |
| 2 | 资深校对编辑（按目标语言母语级标准） | 语法用词、术语统一、数字单位不漂移 |
...
- **不绑定特定语言**：角色能力按「目标语言母语级标准」定义，不绑定中英。支持任意语言对、任意方向（含外→中、中文站内多语言）；法域规则必须按项目配置启用。
...
| `references/industry-labels.md` | 受管制品类强制标签（食品过敏原、化妆品成分命名与 INCI 对照、CLP、纺织纤维、WEEE、原产地）与销售国官方语言要求（法国 Loi Toubon、魁北克 Bill 96 等） |
...
如果 Agent 无法自动安装，将下面的 `/path/to/your-agent/skills` 替换为目标 Agent 的 skills 目录：
...
宿主完成 Skill 加载后，说「翻译 / 汉化 / 本地化 / 翻成德语日语 / localize」即可触发；具体触发方式以宿主实现为准。
...
1. **翻译 ≠ 本地化。** 字面忠实只是及格线。
2. **低风险信息不足时先跑再问**：按最可能的假设产出，并把假设写进「待确认」；投放、上架、付款、医疗、电气、法律、标签或其他受监管任务缺少关键信息时，先输出 `BLOCKED`，不要擅自假设。
